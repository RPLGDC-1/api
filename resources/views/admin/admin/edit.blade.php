      ...
      <div class="row">
        <div class="col-md-12">
          <div class="date-picker">
            <form class="theme-form" action="{{ route('admins.update', $admin->id) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <div class="mb-3 row g-3">
                ...
                <div class="col-xl-5 col-sm-9">
                  <div class="input-group">
                    <input class="form-control @error('name') is-invalid border-danger @enderror" name="name" type="text" placeholder="Enter name" value="{{ old('name', $admin->name) }}">
                    @error('name')
              ...
              <div class="mb-3 row g-3">
                ...
                <div class="col-xl-5 col-sm-9">
                  <div class="input-group">
                    <input class="form-control @error('email') is-invalid border-danger @enderror" name="email" type="email" placeholder="Enter email" value="{{ old('email', $admin->email) }}" readonly>
                    @error('email')
                      <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                  </div>
                  ...
                  <div class="input-group">
                    <select class="form-select @error('role_id') is-invalid border-danger @enderror" name="role_id">
                      ...
                      @foreach ($roles as $row)
                        <option value="{{ $row->id }}" {{ $row->id == old('role_id', $admin->role_id) ? 'selected' : '' }}>{{ $row->name }}</option>
                      @endforeach
                    </select>
                    @error('role_id')
                      <div class="invalid-feedback">{{ $message }}</div>
              ...
              <div class="mb-3 row g-3">
                ...
                <div class="col-xl-5 col-sm-9 text-danger">
